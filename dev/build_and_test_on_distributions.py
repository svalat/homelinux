...
######################################################
'''
Test building Homelinux + running unit tests on various distributions via
...
###########################################################
def get_homelinux_source_path() -> str:
...
class PodmanContainerHandler:
    ...
    def start(self):
        host_sources_path = get_homelinux_source_path()
        cmd = ['podman', 'run', '--detach', '--volume', f'{host_sources_path}:/mnt/homelinux-sources:rw', '--rm', '--tty', self.image, "/bin/bash"]
        print(f"Running : {cmd}")
...
######################################################
BUILD_PARAMETERS_ALL = {
    "distributions": {
        "ubuntu:20.04": [
            "apt update",
            "apt upgrade -y",
            "apt install -y cmake make gcc g++ wget curl nodejs wget xz-utils"
        ],
        "ubuntu:22.04": [
            "apt update",
            "apt upgrade -y",
...
def test_prep_image(dist_name_version):
    ...
    # build
    container = PodmanContainerHandler(f"homelinux/{dist_name_version}")
